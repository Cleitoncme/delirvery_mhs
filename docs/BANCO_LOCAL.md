...
A migration `003_admin_sessions.sql` adiciona usuários administrativos, sessões, limite de login, acessos privados de acompanhamento e identificação do operador no histórico. `004_catalog_versions.sql` adiciona controle otimista de edição do catálogo. `005_order_operations.sql` adiciona motivo de cancelamento e índice operacional. Para aplicar em volume existente, execute cada arquivo pendente uma única vez e veja [ADMIN_LOCAL.md](ADMIN_LOCAL.md).
