...
const addressSchema = z.object({
  street: z.string().trim().min(2, "Informe a rua.").max(100),
...
export function CheckoutView({ tenant }: { tenant: Tenant }) {
  ...
  function validate() {
    ...
    const addressResult =
      fulfillment === "DELIVERY" ? addressSchema.safeParse(address) : undefined;
    const next: Errors = {};
    if (!result.success)
    ...
    if (addressResult && !addressResult.success)
      for (const issue of addressResult.error.issues)
        next[`address.${String(issue.path[0])}`] = issue.message;
    const change = Number(changeFor.replace(",", "."));
    if (
      payment === "CASH" &&
      changeFor &&
      (!Number.isFinite(change) || change * 100 < subtotal + fee)
    )
      next.changeFor = "O troco deve ser maior ou igual ao total.";
    setErrors(next);
    ...
    <>
      ...
      <div className="checkout-grid">
        ...
        >
            ...
            <div className="two-fields">
              ...
              <Field
                ...
                onChange={(v) =>
                  setCustomer({
                    ...customer,
                    phone: v.replace(/[^\d()\s-]/g, "").slice(0, 16),
                  })
                }
              />
            </div>
          ...
          <section className="panel">
            ...
            <div className="method-grid">
              <Choice
                ...
                text={`${money(tenant.deliveryFee)} • 30 a 60 min`}
                checked={fulfillment === "DELIVERY"}
                onChange={() => setFulfillment("DELIVERY")}
              />
              <Choice
                title="Retirar na loja"
                text="Grátis • 15 a 30 min"
                ...
                <div className="two-fields">
                  ...
                  <Field
                    ...
                    onChange={(v) =>
                      setAddress({
                        ...address,
                        zipCode: v.replace(/\D/g, "").slice(0, 8),
                      })
                    }
                  />
                </div>
                <div className="two-fields">
                  ...
                  <Field
                    ...
                    onChange={(v) =>
                      setAddress({
                        ...address,
                        state: v.replace(/[^a-zA-Z]/g, "").slice(0, 2),
                      })
                    }
                  />
                </div>
          ...
          <section className="panel">
            <h2>Pagamento</h2>
            <div className="method-grid payment-grid">
              <Choice
                title="PIX"
                text="Simulado neste protótipo"
                checked={payment === "PIX"}
                onChange={() => setPayment("PIX")}
              />
              <Choice
                title="Cartão na entrega"
                text="Pague ao receber"
          ...
          <button className="button wide" type="submit">
            Revisar pedido <span>{money(subtotal + fee)}</span>
          </button>
        </form>
      ...
      {showConfirm && (
        <div className="modal-backdrop" role="presentation">
          ...
          >
            ...
            <h2 id="confirm-title">Confirme seu pedido</h2>
            <p>
