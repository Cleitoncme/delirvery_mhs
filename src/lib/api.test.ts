...
it("limita o corpo real sem confiar em Content-Length", async () => {
  const request = new Request("http://localhost/api", {
    ...
    body: JSON.stringify({ text: "a".repeat(65536) }),
  });
...
it("rejeita JSON inválido e aceita JSON com charset", async () => {
  ...
  await expect(jsonRequest(request("{"))).rejects.toMatchObject({
    status: 400,
  });
  await expect(jsonRequest(request('{"ok":true}'))).resolves.toEqual({
    ok: true,
  });
});
